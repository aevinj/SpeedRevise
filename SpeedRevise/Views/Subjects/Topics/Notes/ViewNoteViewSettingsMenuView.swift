//
//  ViewNoteViewSettingsMenuView.swift
//  SpeedRevise
//
...
struct ViewNoteViewSettingsMenuView: View {
    ...
    var body: some View {
        HStack {
            if !showLoading {
                ...
                Text("Delete \(currQuiz.name.capitalizedFirst)'s note?")
                    .foregroundStyle(Color.red)
            } else {
                ProgressView(label: {
                    Text("")
        ...
        .onTapGesture {
            Task {
                showLoading = true
                await subjectViewModel.deleteNote(subjectID: currSubjectID, topicID: currTopicID, quizID: currQuiz.id)
                showLoading = false
                noteDeleted = true
                dismiss()
            }
